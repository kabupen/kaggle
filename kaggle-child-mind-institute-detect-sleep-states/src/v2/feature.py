...
class Dataset:
    ...
    def make_series_features(self, series_id):
        ...
        # target
        tmp_df = deepcopy(self.df_series[self.df_series[self.id_key]==series_id])

        # cleaning
        tmp_df = tmp_df.dropna(axis=0, ignore_index=True)
        if self.mode == "trian":
            # dropna によって night (ペア番号) が片方なくなるような行が生じてしまうため、該当のペアも削除する
            event_count = self.df_events[[self.id_key, "night", "event"]].groupby(by=[self.id_key, "night"]).count()
            ...
            for series_id, night in zip(event_count[event_count["event"] != 2][self.id_key], event_count[event_count["event"] != 2]["night"]):
                self.df_events = self.df_events[~( (self.df_events[self.id_key]==series_id) & (self.df_events["night"]==night) )].reset_index(drop=True)
        ...
        # diff
        periods = 20
        tmp_df.loc[:, "anglez_diff"] = tmp_df.groupby(self.id_key)['anglez'].diff(periods=periods).bfill().astype('float16')
        tmp_df.loc[:, "enmo_diff"] = tmp_df.groupby(self.id_key)['enmo'].diff(periods=periods).bfill().astype('float16')
        # rolling (移動平均)
        ...
        # (2) events
        if self.mode == "train" :
            tmp_events = deepcopy(self.df_events[self.df_events[self.id_key]==series_id])
